...
public class FWRInfo {
    ...
    public FWRInfo(byte[] message) {
        ...
        if (maybeDestX != -1){
            ...
            System.arraycopy(message, sizeInt* 6 + idLength, content, 0 , message.length-(sizeInt* 6 + idLength+1));
        }
        else {
            int seqNumber = bbuf.getInt();
            int idLength = bbuf.getInt();
           ...
           // System.out.println("ID: " + new String(idArray));
            ...
            System.arraycopy(message, sizeInt* 4 + idLength, content, 0 , message.length-(sizeInt* 4 + idLength+1));
        }
    }
